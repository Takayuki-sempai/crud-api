...
export const Server = () => {
  ...
  const server = http.createServer(
    async (request: IncomingMessage, response: ServerResponse) => {
      ...
      try {
        const method = ensureExists(
          request.method,
          () => `Can't handle request without method url: ${request.url}`,
        );
        ...
        const url = ensureExists(
          request.url,
          () => `url not found in request ${request}`,
        );
  ...
  const prepareRoute = (url: string, handler: UrlHandler): Route => {
    ...
    const testUrl = new RegExp(
      '^' + url.replaceAll('/', '\\/').replaceAll(regexp, '([^\\/]*)') + '$',
      'g',
    );
    return { testUrl, params, handler };
  };

  const createFromBodyHandler =
    (handler: UrlBodyHandler): UrlHandler =>
    async (
  ...
  const post = (url: string, handler: UrlBodyHandler) => {
    methodRoutes['POST'].push(
      prepareRoute(url, createFromBodyHandler(handler)),
    );
  };
